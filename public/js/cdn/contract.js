...
var App = {
    ...
    loadContract: async () => {
        ...
        const landRegistry = await $.getJSON('contract/contrat_test.json')
    ...
    registerLand: async (data) => {
        ...
        await App.landRegistry.register(Number(townshipId), Number(owner_nid), owner_name, owner_address, owner_father_name, owner_mother_name);
        alert('Successfully registered the land.')
    },
    approuveLand: async (data) => {
        let owner_nid = $('[name="owner_nid"]').val();
        // await App.landRegistry.registerTownShip(Number(townshipId), Number(owner_nid), owner_name, owner_address, owner_father_name, owner_mother_name)
        await App.landRegistry.approuve("0xc6e07189bc7565D53CE1C7744879620cCC70C17F",Number(owner_nid));
        alert('Successfully registered the land.')
    },
